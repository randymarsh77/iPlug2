...
def  main():
  ...
  installer = "\installer\DankKnob Installer.exe"
   
  if demo:
    installer = "\installer\DankKnob Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\DankKnob manual.pdf" 
  ]
  ...
  ZIPNAME = "DankKnob-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "DankKnob-v" + FULLVERSIONSTR + "-win-demo.zip"
